			...
			if($this->callback_insert === null)
			{
				...
				foreach($add_fields as $num_row => $field)
				{
					...
					if(array_key_exists($field->field_name, $post_data) && !isset($this->relation_n_n[$field->field_name]))
					{
						...
						elseif(isset($types[$field->field_name]->db_null) && $types[$field->field_name]->db_null && trim($post_data[$field->field_name]) === '')
						{
							$insert_data[$field->field_name] = null;
						}
						elseif(isset($types[$field->field_name]->crud_type) && $types[$field->field_name]->crud_type == 'date')
						{
							$insert_data[$field->field_name] = $this->_convert_date_to_sql_date(trim($post_data[$field->field_name]));
						}
						...
						{
							$insert_data[$field->field_name] = !empty($post_data[$field->field_name]) ? implode(',',trim($post_data[$field->field_name])) : '';
						}
						elseif(isset($types[$field->field_name]->crud_type) && $types[$field->field_name]->crud_type == 'datetime'){
							$insert_data[$field->field_name] = $this->_convert_date_to_sql_date(substr(trim($post_data[$field->field_name]),0,10)).
																		substr(trim($post_data[$field->field_name]),10);
						}
						else
						{
							$insert_data[$field->field_name] = trim($post_data[$field->field_name]);
						}
					}
				}
			...
			if($this->callback_update === null)
			{
				...
				foreach($edit_fields as $num_row => $field)
				{
					...
					if(array_key_exists($field->field_name, $post_data) && !isset($this->relation_n_n[$field->field_name]))
					{
						...
						elseif(isset($types[$field->field_name]->crud_type) && $types[$field->field_name]->crud_type == 'date')
						{
							$update_data[$field->field_name] = $this->_convert_date_to_sql_date(trim($post_data[$field->field_name]));
						}
						elseif(isset($types[$field->field_name]->crud_type) && $types[$field->field_name]->crud_type == 'readonly')
						{
							//This empty if statement is to make sure that a readonly field will never inserted/updated
						...
						elseif(isset($types[$field->field_name]->crud_type) && ($types[$field->field_name]->crud_type == 'set' || $types[$field->field_name]->crud_type == 'multiselect'))
						{
							$update_data[$field->field_name] = !empty($post_data[$field->field_name]) ? implode(',',trim($post_data[$field->field_name])) : '';
						}
						elseif(isset($types[$field->field_name]->crud_type) && $types[$field->field_name]->crud_type == 'datetime'){
							$update_data[$field->field_name] = $this->_convert_date_to_sql_date(substr(trim($post_data[$field->field_name]),0,10)).
																		substr(trim($post_data[$field->field_name]),10);
						}
						else
						{
							$update_data[$field->field_name] = trim($post_data[$field->field_name]);
						}
					}
				}
