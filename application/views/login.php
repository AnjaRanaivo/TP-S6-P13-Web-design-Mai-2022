<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link href="<?php echo assetsBack_url("/css/bootstrap.css") ?>" rel="stylesheet" />
        <link href="<?php echo assetsBack_url("/css/stylelogin.css") ?>" rel="stylesheet" />
    <title>Login Back</title>
</head>
<body>
<!-- Pills navs -->
<section class="vh-100 gradient-custom">
  <div class="container py-5 h-100">
    <div class="row d-flex justify-content-center align-items-center h-100">
      <div class="col-12 col-md-8 col-lg-6 col-xl-5">
        <div class="card bg-dark text-white" style="border-radius: 1rem;">
          <div class="card-body p-5 text-center">

            <div class="mb-md-5 mt-md-4 pb-5">

              <h2 class="fw-bold mb-2 text-uppercase">Se connecter</h2>
              <p class="text-white-50 mb-5">Entrez votre email et mot de passe</p>
            <form action="<?php echo site_url("LoginController/loginSimple"); ?>" method="post">
              <div class="form-outline form-white mb-4">
                <input name="email" type="email" id="typeEmailX" class="form-control form-control-lg" />
                <label class="form-label" for="typeEmailX">Email</label>
              </div>

              <div class="form-outline form-white mb-4">
                <input name="mdp" type="password" id="typePasswordX" class="form-control form-control-lg" />
                <label class="form-label" for="typePasswordX">Mot de Passe</label>
              </div>

              
                <?php if(isset($error)){ ?>
                    <div>
                <p class="mb-0" style="color:red"><?php echo $error; ?>
                </p>
                </div>
                <?php } ?>

              <button class="btn btn-outline-light btn-lg px-5" type="submit">Se connecter</button>
            </form>


            </div>


          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- Pills content -->
</div>
</body>
</html>